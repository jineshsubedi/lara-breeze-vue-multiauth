...
class MyTrainingController extends Controller
{
    public function index()
    {
        ...
        return Inertia::render('Supervisor/Mytrainings/Index', [
    ...
    public function show($id)
    {
        ...
        $participants = TrainingParticipant::with(['user:id,name'])
            ...
            ->latest()
            ->get();
        return Inertia::render('Supervisor/Mytrainings/Show', [
            'training' => $training,
    ...
    public function evaluationForm($id)
    {
        ...
        $questions = TrainingTemplateQuestion::where('training_template_id', $template->id)
                ->orderBy('order')
                ->get();
        return Inertia::render('Supervisor/Mytrainings/Evaluation', [
            'template' => $template,
            'questions' => $questions,
    ...
    public function saveEvaluationForm($id, Request $request)
    {
        ...
        return redirect()->route('supervisor.mytrainings.index', ['type'=>'evaluation'])->with('success', 'Form Submitted Successfully');
    }

...
function destroy(id) {
    if (confirm("Are you sure you want to Delete")) {
        form.delete(route("staffs.dailytasks.destroy", id));
    }
}
function filterDailyTask()
{
    Inertia.get(
        route('staffs.dailytasks.index'),
...
function approveDTask() {
    approveForm.post(route("staffs.dailytasks.approve"), {
        preserveScroll: true,
        onSuccess: () => {
            // approveForm.reset();
        },
        ...
        <template #breadcrum>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">
                    <Link :href="route('staffs.dailytasks.index')" :only="['dtasks']"> Daily Tasks </Link>
                </li>
            </ol>
        </template>
        <div class="container">
            ...
            <AddTodayTask :url="route('staffs.dailytasks.store')" :kras="kras"/>
            <div class="card">
                <div class="card-body">
                    <h5 class="card-title">Daily Tasks</h5>
                    <ul class="nav nav-tabs">
                        <li class="nav-item">
                            <NavLink :active="filters.type == 0 || filters.type == null" :href="route('staffs.dailytasks.index',{type: 0})" >All</NavLink>
                        </li>
                        <li class="nav-item">
                            <NavLink :active="filters.type == 1" :href="route('staffs.dailytasks.index', {type:1})" >My Daily Work</NavLink>
                        </li>
                        <li class="nav-item">
                            <NavLink :active="filters.type == 2" :href="route('staffs.dailytasks.index', {type:2})">Daily Work Approval <span class="badge bg-danger">{{aprovalCount}}</span></NavLink>
                        </li>
                    </ul>
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <td scope="row">
                                    <Link :href="route('staffs.dailytasks.show', dtask.id)"
                                        class="btn btn-sm btn-outline-info">
                                        <i class="bi bi-eye"></i>
                                    </Link>
                                    <div v-if="dtask.status == '0'">
                                        <div class="btn-group" v-if="dtask.user_id == $page.props.auth.user.id">
                                            <Link :href="route('staffs.dailytasks.edit', dtask.id)"

...
function destroy(id) {
    if (confirm("Are you sure you want to Delete")) {
        form.delete(route("staffs.bookings.destroy", id));
    }
}
</script>
<template>
        ...
        <template #breadcrum>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">
                    <Link :href="route('staffs.bookings.index')" :only="['bookings']"> Booking </Link>
                </li>
            </ol>
        </template>
        <div class="container">
            <div class="text-right">
                <Link :href="route('staffs.bookings.create')" class="btn btn-sm btn-outline-info">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <ul class="nav nav-tabs">
                        <li class="nav-item">
                            <NavLink :active="filters.type == 0 || filters.type == null" :href="route('staffs.bookings.index',{type: 0})" >All Booking</NavLink>
                        </li>
                        <li class="nav-item">
                            <NavLink :active="filters.type == 1" :href="route('staffs.bookings.index', {type:1})" >My Booking</NavLink>
                        </li>
                    </ul>
                            ...
                            >
                                ...
                                <td scope="row">
                                    <div class="btn-group" v-if="booking.booked_by == $page.props.auth.user.id">
                                        <Link :href="route('staffs.bookings.show', booking.id)"
                                            class="btn btn-sm btn-outline-info">
                                            <i class="bi bi-eye"></i>
                                        </Link>
                                        <Link :href="route('staffs.bookings.getParticipants', booking.id)"
                                            class="btn btn-sm btn-outline-success">
                                            <i class="bi bi-people-fill"></i>
                                        </Link>
                                        <Link :href="route('staffs.bookings.edit', booking.id)"

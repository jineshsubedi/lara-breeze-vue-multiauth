...
function destroy(id) {
    if (confirm("Are you sure you want to Delete")) {
        form.delete(route("supervisor.travelplanners.destroy", id));
    }
}
</script>
<template>
        ...
        <template #breadcrum>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">
                    <Link :href="route('supervisor.travelplanners.index')" :only="['travelplanners']"> TravelPlanner </Link>
                </li>
            </ol>
        </template>
        <div class="container">
            <div class="text-right">
                <Link :href="route('supervisor.travelplanners.create')" class="btn btn-sm btn-outline-info">
                ...
                <ul class="nav nav-tabs">
                    <li class="nav-item" v-if="SuperAdmin || HrHandler">
                        <ReportLink
                            ...
                            "
                            :href="
                                route('supervisor.travelplanners.index', [
                                    { tab: 'all' },
                                ])
                            "
                            ...
                            >
                                ...
                                <td scope="row">
                                    <div class="btn-group" v-if="filters.tab == null">
                                        <Link :href="route('supervisor.travels.show', travelplanner.travel_id)"
                                            class="btn btn-sm btn-outline-info">
                                            <i class="bi bi-eye"></i>
                                        </Link>
                                        <Link :href="route('supervisor.travelplanners.edit', travelplanner.id)"
                                    ...
                                    <div class="btn-group" v-if="filters.tab == 'waiting'">
                                        ...
                                        <Link :href="route('supervisor.travelplanners.approval', travelplanner.id)" method="post" :data="{ action: 'Approve' }" class="btn btn-sm btn-outline-success">Approve</Link>
                                        <Link :href="route('supervisor.travelplanners.approval', travelplanner.id)" method="post" :data="{ action: 'Reject' }" class="btn btn-sm btn-outline-danger">Reject</Link>

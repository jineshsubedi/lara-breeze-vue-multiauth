...
function destroy(id) {
    if (confirm("Are you sure you want to Delete")) {
        form.delete(route("supervisor.leaves.destroy", id));
    }
}
function approvalLeaveRequest(id, supervisor, hr, manager) {
    ...
    approveForm.patch(route("supervisor.leaves.approval", id));
}
</script>
<template>
        ...
        <template #breadcrum>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">
                    <Link :href="route('supervisor.leaves.index')"> Leave Request </Link>
                </li>
            </ol>
        </template>
        <div class="container">
            <div class="text-right">
                <Link :href="route('supervisor.leaves.create')" class="btn btn-sm btn-outline-info">
            ...
            <div class="card">
                <div class="card-body">
                    <ul class="nav nav-tabs">
                        <li class="nav-item">
                            <NavLink :active="filters.type == 0 || filters.type == null" :href="route('supervisor.leaves.index',{type: 0})" >All</NavLink>
                        </li>
                        <li class="nav-item">
                            <NavLink :active="filters.type == 1" :href="route('supervisor.leaves.index', {type:1})" >My Requests</NavLink>
                        </li>
                        <li class="nav-item" v-if="leaveSetting.s_approval == '1'">
                            <NavLink :active="filters.type == 2" :href="route('supervisor.leaves.index', {type:2})">Approval <span class="badge bg-danger">{{countData.s_count}}</span></NavLink>
                        </li>
                        <li class="nav-item" v-if="leaveSetting.h_approval == '1' && leaveSetting.hr_person == $page.props.auth.user.id">
                            <NavLink :active="filters.type == 3" :href="route('supervisor.leaves.index', {type:3})">HR Approval <span class="badge bg-danger">{{countData.h_count}}</span></NavLink>
                        </li>
                        <li class="nav-item" v-if="leaveSetting.m_approval == '1' && leaveSetting.m_person == $page.props.auth.user.id">
                            <NavLink :active="filters.type == 4" :href="route('supervisor.leaves.index', {type:4})">Chairman Approval <span class="badge bg-danger">{{countData.m_count}}</span> </NavLink>
                        </li>
                    </ul>
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                >
                                    ...
                                    <td scope="row">
                                        <div class="btn-group">
                                            <Link :href="route('supervisor.leaves.show', leave.id)"

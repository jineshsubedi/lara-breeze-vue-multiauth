...
{
    Inertia.get(
        route('supervisor.helpdesks.index'),
...
function destroy(id) {
    if (confirm("Are you sure you want to Delete")) {
        form.delete(route("supervisor.helpdesks.destroy", id));
    }
}
</script>
<template>
        ...
        <template #breadcrum>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">
                    <Link :href="route('supervisor.helpdesks.index')" :only="['tasks']"> Help Desk </Link>
                </li>
            </ol>
        </template>
        <div class="container">
            <div class="text-right">
                <Link :href="route('supervisor.helpdesks.create')" class="btn btn-sm btn-outline-info">
            ...
            <div class="card">
                <div class="card-body">
                    <ul class="nav nav-tabs">
                        <li class="nav-item">
                            <NavLink :active="filters.type == 0 || filters.type == null" :href="route('supervisor.helpdesks.index',{type: 0})">All</NavLink>
                        </li>
                        <li class="nav-item">
                            <NavLink :active="filters.type == 1" :href="route('supervisor.helpdesks.index', {type:1})">TO DO</NavLink>
                        </li>
                        <li class="nav-item">
                            <NavLink :active="filters.type == 2" :href="route('supervisor.helpdesks.index', {type:2})">My Help Desk</NavLink>
                        </li>
                        <li class="nav-item">
                            <NavLink :active="filters.type == 3" :href="route('supervisor.helpdesks.index', {type:3})">Help Desk Assigned</NavLink>
                        </li>
                    </ul>
                        ...
                        <table class="table table-striped">
                            ...
                            <tbody>
                                ...
                                >
                                    ...
                                    <td scope="row">
                                        <div class="btn-group">
                                            <Link :href="route('supervisor.helpdesks.acceptTask', task.id)" class="btn btn-sm btn-outline-success" v-if="$page.props.auth.user.id == task.task_to && task.accept_status == '0'">
                                                <i class="bi bi-check-lg"></i> Accept
                                            </Link>
                                            <Link :href="route('supervisor.helpdesks.completeTask', task.id)" class="btn btn-sm btn-outline-success" v-if="$page.props.auth.user.id == task.task_to && task.accept_status == '1' && task.complete_status == '0'">
                                                <i class="bi bi-check-lg"></i> Complete
                                            </Link>
                                            <Link :href="route('supervisor.helpdesks.show', task.id)"
                                                class="btn btn-sm btn-outline-info">
                                                <i class="bi bi-eye"></i> View
                                            </Link>
                                            <Link :href="route('supervisor.helpdesks.edit', task.id)"

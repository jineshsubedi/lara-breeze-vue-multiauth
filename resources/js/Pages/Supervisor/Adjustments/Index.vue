...
function destroy(id) {
    if (confirm("Are you sure you want to Delete")) {
        form.delete(route("supervisor.adjustments.destroy", id));
    }
}
function sApproveForm(id)
{
    if (confirm("Are you sure you want to Approve")) {
        sApproveform.patch(route("supervisor.adjustments.status", id));
    }
}
function hrApproveForm(id)
{
    if (confirm("Are you sure you want to Approve")) {
        hrApproveform.patch(route("supervisor.adjustments.status", id));
    }
}
function rejectAdjustmentForm(id)
{
    if (confirm("Are you sure you want to Reject")) {
        rejectForm.patch(route("supervisor.adjustments.status", id));
    }
}
...
{
    Inertia.get(
        route('supervisor.adjustments.index'),
        ...
        <template #breadcrum>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">
                    <Link :href="route('supervisor.adjustments.index')" :only="['adjustments']"> Adjustment </Link>
                </li>
            </ol>
        </template>
        <div class="container">
            <div class="text-right">
                <Link :href="route('supervisor.adjustments.create')" class="btn btn-sm btn-outline-info">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <ul class="nav nav-tabs">
                        <li class="nav-item">
                            <NavLink :active="filters.type == 0 || filters.type == null" :href="route('supervisor.adjustments.index',{type: null})" >My Adjustments</NavLink>
                        </li>
                        <li class="nav-item">
                            <NavLink :active="filters.type == 1" :href="route('supervisor.adjustments.index', {type:1})" >SubOrdinate Adjustments</NavLink>
                        </li>
                        <li class="nav-item" v-if="HrHandler">
                            <NavLink :active="filters.type == 2" :href="route('supervisor.adjustments.index', {type:2})">Staffs Adjustment</NavLink>
                        </li>
                    </ul>
                    <div class="row mt-3">
                            ...
                            >
                                ...
                                <td scope="row">
                                    <div class="btn-group"  v-if="adjustment.user_id == $page.props.auth.user.id">
                                        <Link :href="route('supervisor.adjustments.show', adjustment.id)"
                                            class="btn btn-sm btn-outline-info">
                                            <i class="bi bi-eye"></i>
                                        </Link>
                                        <Link :href="route('supervisor.adjustments.edit', adjustment.id)"

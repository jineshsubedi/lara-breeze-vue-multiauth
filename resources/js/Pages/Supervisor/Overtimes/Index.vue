...
function destroy(id) {
    if (confirm("Are you sure you want to Delete")) {
        form.delete(route("supervisor.overtimes.destroy", id));
    }
}
function sApproveForm(id)
{
    if (confirm("Are you sure you want to Approve")) {
        sApproveform.patch(route("supervisor.overtimes.status", id));
    }
}
function hrApproveForm(id)
{
    if (confirm("Are you sure you want to Approve")) {
        hrApproveform.patch(route("supervisor.overtimes.status", id));
    }
}
function rejectovertimeForm(id)
{
    if (confirm("Are you sure you want to Reject")) {
        rejectForm.patch(route("supervisor.overtimes.status", id));
    }
}
...
{
    Inertia.get(
        route('supervisor.overtimes.index'),
        ...
        <template #breadcrum>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">
                    <Link :href="route('supervisor.overtimes.index')" :only="['overtimes']"> Overtime </Link>
                </li>
            </ol>
        </template>
        <div class="container">
            <div class="text-right">
                <Link :href="route('supervisor.overtimes.create')" class="btn btn-sm btn-outline-info">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <ul class="nav nav-tabs">
                        <li class="nav-item">
                            <NavLink :active="filters.type == 0 || filters.type == null" :href="route('supervisor.overtimes.index',{type: null})" >My Overtimes</NavLink>
                        </li>
                        <li class="nav-item">
                            <NavLink :active="filters.type == 1" :href="route('supervisor.overtimes.index', {type:1})" >SubOrdinate Overtimes</NavLink>
                        </li>
                        <li class="nav-item" v-if="HrHandler">
                            <NavLink :active="filters.type == 2" :href="route('supervisor.overtimes.index', {type:2})">Staffs Overtime</NavLink>
                        </li>
                    </ul>
                    <div class="row mt-3">
                            ...
                            >
                                ...
                                <td scope="row">
                                    <div class="btn-group"  v-if="overtime.user_id == $page.props.auth.user.id">
                                        <Link :href="route('supervisor.overtimes.show', overtime.id)"
                                            class="btn btn-sm btn-outline-info">
                                            <i class="bi bi-eye"></i>
                                        </Link>
                                        <Link :href="route('supervisor.overtimes.edit', overtime.id)"

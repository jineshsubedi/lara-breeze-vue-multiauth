...
function destroyJob(id) {
    if (confirm("Are you sure you want to Delete")) {
        form.delete(route("supervisor.taskjobs.delete", id));
    }
}
function submitNewTaskJob() {
    form.post(route('supervisor.taskjobs.save', props.task.id), {
        preserveScroll: true,
        onSuccess: () => {
            form.reset(),
        ...
        <template #breadcrum>
            <ol class="breadcrumb float-sm-right">
                ...
                <li class="breadcrumb-item active">
                    <Link :href="route('supervisor.tasks.show', task.id)"> Detail </Link>
                </li>
            </ol>
        </template>
        <div class="">
                            ...
                            <table class="table table-bordered" v-if="task.jobs_count > 0">
                                ...
                                <tbody>
                                    <tr v-for="(jtask, index) in task.jobs" :key="index">
                                        ...
                                        <td>
                                            <Link :href="route('supervisor.taskjobs.acceptTask', jtask.id)" class="btn btn-sm btn-outline-success" v-if="$page.props.auth.user.id == task.task_to && jtask.complete_status[0] != 1">

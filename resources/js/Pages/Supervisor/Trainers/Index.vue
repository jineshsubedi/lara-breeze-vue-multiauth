...
watch(name, throttle(function (value){
    Inertia.get(
        route('supervisor.trainers.index'),
        { name: name.value},
        {
            preserveState: true,
            preserveScroll: true,
...
function destroy(id) {
    if (confirm("Are you sure you want to Delete")) {
        form.delete(route("supervisor.trainers.destroy", id));
    }
}
</script>
<template>
        ...
        <template #breadcrum>
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active">
                    <Link :href="route('supervisor.trainers.index')" :only="['trainers']"> Trainer </Link>
                </li>
            </ol>
        </template>
        <div class="container">
            <div class="text-right">
                <Link :href="route('supervisor.trainers.create')" class="btn btn-sm btn-outline-info">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <td scope="row">
                                    <div class="btn-group">
                                        <Link :href="route('supervisor.trainers.show', trainer.id)"
                                            class="btn btn-sm btn-outline-info">
                                            <i class="bi bi-eye"></i>
                                        </Link>
                                        <Link :href="route('supervisor.trainers.edit', trainer.id)"
